...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            block.time = currentDate.getTime();
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let timeDiff = currentTime - blockTime;
            if (timeDiff <= (5 * 60 * 1000)) {
                ...
                reject('Star not submitted within 5 minutes');
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           let resultBlock = self.chain.filter(block => block.hash == hash)[0];
           if (resultBlock) {
               resolve(resultBlock);
           } else {
               reject('Invalid Hash in chain');
           }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach(async block => {
                ...
                if (index > 0) {
                    ...
                    let actualPreviousHash = self.chain[index - 1].hash;
                    if(actualPreviousHash != previousBlockHash){
                        errorLog.push(`Error - Block ${index} - Previous Hash mismatch.`);
                    }
                }
                index++;
            });
